![alt text](sc-6.png)# Build Plan — saaskaya
...
**Spec:** `docs/specs/2026-07-08-two-layer-ai-gatekeeper.md`

**Why:** every chat message currently calls Claude Opus (~$0.10–0.30/msg). Off-topic messages burn
the tenant's monthly budget for nothing. A conversational onboarding + approval card makes AI edits
transparent, reviewable, and cheaper. Opens monetization (top-up +€10, human help €25/session).

**Architecture:** Layer 1 (gatekeeper — Groq Llama 3.3 70B free tier, Claude Haiku fallback)
classifies intent + distills the request into a structured prompt + assesses risk. User approves.
Layer 2 (Claude Opus, existing `patch_site` / `create_site`) runs only on approval. Constitution §2
unchanged for Layer 2; Layer 1 is provider-agnostic (amendment logged in `PROGRESS.md`).
...
**Spec:** `docs/specs/2026-07-08-beta-launch-hybrid-onboarding.md`

**Why:** transition from seed-tenant MVP to real users. A closed beta (invite-only, selected
professional groups) bounds AI cost and domain-registration risk. Hostinger SMTP gets email working
today (Resend is a one-setting switch later). Bank transfer (EFT/havale) is the most trusted payment
in Turkey for small businesses; Stripe is added for international/production. Domain reservation +
operator-confirmed payment preserves constitution §5 (registration only after payment).

**Phases:**

- **Phase 1** — beta allowlist (`BETA_ALLOWLIST`) + Hostinger SMTP provider (`nodemailer`) + login gate.
