...
## 2026-07-17 — Kullanıcı paneli layout sözleşmesi hizalandı

- Root cause: `/dashboard`, `/account`, support ve mesaj ekranları `PageShell` üzerinden birbirinden farklı `max-width` ve canvas genişlikleri geçiriyordu. Aynı sidebar frame’i kullanılsa da içerik ritmi ve sağ kolon hizası route bazında değişiyordu.
...
## 2026-07-18 — Homepage visual hierarchy polish

- Fixed the homepage's flat hierarchy: every marketing section now has a real `sk-display` H2 headline under its eyebrow (new `sectionTitles` copy keys in TR/EN/DE), restoring a proper h1→h2→h3 document outline that previously jumped h1→h3.
- Added `.sk-eyebrow` (11px mono, `--sk-muted`) replacing the 10.5px `--sk-faint` labels that failed WCAG AA contrast; `--sk-faint` remains for decorative separators only. Scaled the hero up (h1 to 3.25rem on desktop, lead to 16–17px).
- Deployed the brand teal (`--sk-accent`, new `--sk-accent-soft` token) into supporting roles per the "ink CTAs + teal accents" decision: step/check icons, card action arrows and labels, Pro plan ring, FAQ open marker and summary hover, `sk-link` hover, card hover borders. Primary buttons stay ink.
- CTA discipline: demoted the example-site "Düzenle" buttons from primary to secondary so ink-filled buttons are reserved for real conversion CTAs; unified the beta CTA wording to "Betaya başla" / "Start the beta" / "Beta starten" (was "Beta başlangıcını aç").
- Broke the card-wall monotony with two full-canvas background bands via a new `band` prop on `MarketingSection` (`card` for Process with tone-inverted `sk-soft` cards, `paper` for Trust with `sk-card` cards); consolidated the component's four tag branches into `svelte:element`.
- Localization: preset badges now render localized labels (Hukuk/Psikoloji/Diş etc.) instead of raw seed keys; the three hardcoded English pricing aria-labels are localized (`planAria`). Type-scale bump: card body copy 12px→13px, FAQ answers to 14px, locale chips 10px→11px with taller touch targets.
- Follow-up backlog: roll the same treatment (eyebrow+H2 pattern, teal roles, CTA discipline, type scale) to the other public pages once the homepage result is approved; FlowAnimation's internal English chrome labels were left as-is (they mirror the real editor UI).
- Verification: `npm run check` 0 errors/warnings; production build succeeds; SSR curl checks confirmed all new H2s, badges and CTA labels in TR/EN/DE; real-browser QA (playwright-core) at 1440px (TR+EN) and 375px (TR) found zero horizontal overflow, zero console errors and the expected outline; prettier run on touched files.

## 2026-07-19 — Homepage round 2: hero restructure, full-width footer, dark closing CTA

- Restructured the hero to a centered, text-focused layout and removed the two hero-trust blocks ("AI kod yazmaz" / "Site senin") — the same reassurance already lives in the problem band, Trust section and FAQ. Deleted the `heroTrust` copy keys (not referenced by admin copy overrides) and the unused icon import.
- Moved the FlowAnimation out of the hero into its own centered section (max-w-4xl) directly below, headed by the previously unused `flowLabel` copy ("Canlı akışta gör" / "See it in action" / "In Aktion ansehen"). The animation scales its fixed design stage uniformly and self-pauses off-screen, so the larger placement costs nothing.
- Footer is now full-width: `PublicFooter` moved out of `AppCanvasShell` in `PublicShell` (also fixes semantics — footer no longer inside `<main>`), applies to all public pages; background made solid `--sk-card` and column headings switched to `.sk-eyebrow`.
- Final CTA became the page's single dark moment: new `band="ink"` variant on `MarketingSection` (`--sk-ink` full-canvas band, cream text, inverted primary button via scoped CSS). Content reveals on scroll through a new client-only `reveal` action (`src/lib/actions/reveal.ts`, IntersectionObserver, threshold 0.35, staggered fade-rise) and the bee mascot flies in once then floats gently (`sk-bee-arrive`/`sk-bee-float` keyframes). SSR/no-JS render fully visible content; `prefers-reduced-motion` skips the action and all animations.
- Verification: `npm run check` 0 errors/warnings; production build succeeds. SSR curls: hero-trust copy absent, `sk-band--ink` and flow-label present, footer renders after `</main>`. Playwright at 1440px and 375px: reveal arms and fires on scroll, reduced-motion context shows content immediately with no reveal classes and bee opacity 1, zero horizontal overflow, zero console errors; desktop full-page and CTA-band screenshots visually confirmed. Prettier run on touched files. Still uncommitted alongside round 1 pending user approval.

## 2026-07-19 — Homepage round 3: user visual feedback pass

- Enlarged the brand wordmark in `BrandMark.svelte` (text-sm → text-base, text-xl on desktop) — it was disproportionately small next to the 48px logo; applies to the public header, mobile nav, and auth/onboarding surfaces that render BrandMark.
- Normalized the two sections the user flagged as visually inconsistent: the problem band lost its `sk-soft` container (now a standard eyebrow+H2 section with `sk-card` items) and the Trust section lost its paper band. The inline legal-links row under Trust was removed along with the now-unused `legal` copy objects in all three locales — the footer already carries those links. The page's band rhythm is now: white band (process) + ink band (final CTA) only.
- Hero "Fiyatlandırma" button upgraded ghost → secondary (bordered) for visibility.
