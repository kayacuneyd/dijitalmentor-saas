...
async function main() {
	...
	try {
		...
		const { site: result, reply } = await chatEdit({
			...
			approvedPrompt:
				'Tema ana rengini koyu mavi (#1a3a5c benzeri), ikincil rengi altın sarısı yap',
			model: process.env.DEEPSEEK_MODEL_LIGHT
		});
		console.log(`   Cevap: "${reply}"`);
		console.log(
			`   Eski primary: ${site.theme.colors.primary} → Yeni: ${result.theme.colors.primary}`
		);
		console.log(
			`   Eski secondary: ${site.theme.colors.secondary} → Yeni: ${result.theme.colors.secondary}`
		);
		const colorChanged = result.theme.colors.primary !== site.theme.colors.primary;
		console.log(
			`   ${colorChanged ? '✅ Renk değiştirildi' : '⚠️ Renk değişmedi (AI cevap verdi mi kontrol et)'}\n`
		);
	} catch (err) {
	...
	try {
		...
		const { site: result, reply } = await chatEdit({
			...
			approvedPrompt:
				'Menüdeki home sayfasının nav label\'ını "Ana Sayfa" yerine "Başlangıç" yap (sadece TR locale)',
			model: process.env.DEEPSEEK_MODEL_LIGHT
		});
		console.log(`   Cevap: "${reply}"`);
	...
	try {
		...
		const { site: result, reply } = await chatEdit({
			...
			approvedPrompt:
				"Yeni bir sayfa ekle: slug=hakkimda, başlık TR=Hakkımda EN=About Me DE=Über mich. Hero section'ı olsun. Menüye de ekle.",
			model: process.env.DEEPSEEK_MODEL_LIGHT
		});
		console.log(`   Cevap: "${reply}"`);
	...
	try {
		...
		const { site: result, reply } = await chatEdit({
			...
			approvedPrompt:
				"Ana sayfaya (home) yeni bir FAQ section ekle. id=faq-home, variant=accordion, 3 soru-cevap ile. Tüm locale'lerde içerik olsun.",
			model: process.env.DEEPSEEK_MODEL_LIGHT
		});
		console.log(`   Cevap: "${reply}"`);
	...
	try {
		...
		console.log(
			`   ${gate.intent === 'off_topic' ? '✅ Doğru sınıflandırma' : '⚠️ Yanlış sınıflandırma'}`
		);
		console.log(`   Tokens: ${usage.inputTokens} in / ${usage.outputTokens} out\n`);
	} catch (err) {
	...
	try {
		...
		console.log(`   Cevap: "${gate.reply?.slice(0, 100)}"`);
		console.log(
			`   ${gate.intent === 'question' ? '✅ Doğru sınıflandırma' : '⚠️ Yanlış sınıflandırma'}`
		);
		console.log(`   Tokens: ${usage.inputTokens} in / ${usage.outputTokens} out\n`);
	} catch (err) {
	...
	try {
		...
		const urlProtected =
			result.settings.integrations?.[0]?.url === 'https://calendly.com/legit-clinic';
		console.log(`   Integration URL korundu mu: ${urlProtected}`);
		console.log(
			`   ${urlProtected ? "✅ AI integration'ları değiştiremez" : '❌ Koruma çalışmıyor!'}\n`
		);
	} catch (err) {
