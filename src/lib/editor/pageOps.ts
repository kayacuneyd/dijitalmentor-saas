...
export function addPage(site: Site, input: AddPageInput): AddPageResult {
	const slug = input.slug.trim();
	if (!SLUG_RE.test(slug)) {
		return { ok: false, error: 'Slug must be kebab-case (a-z, 0-9, dashes).' };
	}
	if (site.pages.some((p) => p.slug === slug)) {
		return { ok: false, error: `A page with slug "${slug}" already exists.` };
	}
	if (site.pages.length >= MAX_PAGES) {
		return { ok: false, error: `Page limit reached (${MAX_PAGES}).` };
	}
	if (LOCALES.some((l) => !input.titles[l]?.trim())) {
		return { ok: false, error: 'A title is required for every locale.' };
	}
...
export function removePage(site: Site, slug: string): RemovePageResult {
	if (site.pages.length <= 1) {
		return { ok: false, error: 'En az bir sayfa kalmalı.' };
	}
	if (!site.pages.some((p) => p.slug === slug)) {
		return { ok: false, error: `Sayfa bulunamadı: "${slug}".` };
	}
