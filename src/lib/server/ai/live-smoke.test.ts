...
describe.runIf(RUN_LIVE)('🔍 Canlı AI Chat Smoke', () => {
	...
	it('🎨 DeepSeek — renk değişimi (set_theme)', async () => {
		...
		const { site: result, reply } = await chatEdit({
			...
			approvedPrompt: 'Tema renklerini değiştir: primary rengi koyu lacivert (#1a3a5c gibi), secondary rengi altın sarısı (#b08d57 gibi)',
	...
	it('📐 DeepSeek — layout değişimi (set_layout)', async () => {
		...
		const { site: result, reply } = await chatEdit({
			...
			approvedPrompt: 'Layout değiştir: nav variant drawer, sticky header aktif, container width narrow, section spacing tight',
			model: process.env.DEEPSEEK_MODEL_LIGHT
		});
		console.log('  Reply:', reply);
	...
	it('📄 DeepSeek — sayfa ekleme (add_page)', async () => {
		...
		const { site: result, reply } = await chatEdit({
			...
			approvedPrompt: 'add_page: slug=hakkimda, title TR=Hakkımda EN=About Me DE=Über mich, hero section, addToNav=true',
			model: process.env.DEEPSEEK_MODEL_LIGHT
		});
		console.log('  Reply:', reply);
	...
	it('🧩 DeepSeek — section ekleme (add_section)', async () => {
		...
		const { site: result, reply } = await chatEdit({
			...
			approvedPrompt: 'add_section: pageSlug=home, type=faq, id=faq-home, variant=accordion, 3 soru-cevap tr/en/de',
			model: process.env.DEEPSEEK_MODEL_LIGHT
		});
		console.log('  Reply:', reply);
	...
	it('🔒 Integration protection — AI phishing yapamaz', () => {
		...
		s.settings.integrations = [
			{ enabled: true, type: 'booking-external' as const, url: 'https://calendly.com/legit-clinic', 
				label: { tr: 'Randevu', en: 'Appointment', de: 'Termin' } }
		];
		const result = applyPatch(s, [
			{ op: 'set_settings' as const, settings: { siteName: 'Test' } }
		]);
